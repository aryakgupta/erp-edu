...
This guide is about creating modules for ERP's web client.

To create websites with ERP, see :doc:`website`; to add business capabilities
or extend existing business systems of ERP, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed ERP <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple ERP module holding basic web component
...
You then need to add that folder to ERP's
...
Files used in the "web" side of an ERP module must be placed in a ``static``
...
.. warning::
    ...
    "developer mode": log into your ERP instance (user *admin* password
    *admin* by default) open the user menu (in the top-right corner of the
    ERP screen) and select :guilabel:`About ERP` then :guilabel:`Activate
    the developer mode`:
...
The ERP framework uses one such pattern to define modules within web addons,
...
In ERP web, modules are declared as functions set on the global ``openerp``
...
* the first parameter is the current instance of the ERP web client, it gives
  access to various capabilities defined by the ERP (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the ERP web client needs to call them
...
For simplicity and developer-friendliness ERP web provides a class
...
The ERP web client bundles jQuery_ for easy DOM manipulation. It is useful
...
ERP Web makes specific components responsible for sections of a page. In
ERP web, the base for such components is the :class:`~openerp.Widget`
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear ERP user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in ERP Web, you will need to reload
    the page. There is no need to restart the ERP server.

The ``HomePage`` widget is used by ERP Web and managed automatically.
...
As many other environments, ERP's solution is to use a `template engine`_.
ERP's template engine is called :ref:`reference/qweb`.
...
* It has special support in ERP Web's :class:`~openerp.Widget`, though it
  can be used outside of ERP's web client (and it's possible to use
...
.. note::
    ...
    like ERP :ref:`views <reference/views>`.
    ...
    characteristic of ERP, and losing it was considered unacceptable.
...
The class system of the ERP web framework allows direct modification of
...
In ERP, translations files are automatically generated by scanning the source
...
Communication with the ERP Server
==================================

Contacting Models
-----------------

Most operations with ERP involve communicating with *models* implementing
...
communicating with ERP requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, ERP web provides
...
The class used to call ERP models is :class:`openerp.Model`. It is
instantiated with the ERP model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~openerp.web.Model.call` can be used to call any (public) method of an
ERP model. It takes the following positional arguments:
...
the time zone of the user, used to compute correctly dates and times if ERP
...
are related to the configuration of the current user in ERP plus the
...
While :func:`~openerp.Model.call` is sufficient for any interaction with ERP
models, ERP Web provides a helper for simpler and clearer querying of models
...
.. exercise:: Pet Toys List
    ...
    In ERP, images are generally stored in regular fields encoded as
...
In ERP, many operations start from an :ref:`action <reference/actions>`:
...
In ERP Web, the component responsible for handling and reacting to these
...
the ERP web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for ERP web. The server simply sends
...
Much of ERP web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`ERP views <reference/views>` are implemented through a subclass
...
The *search view* is considered a view type by the main ERP framework, but
...
* It must correctly implement the 3 field attributes available in all fields
  of ERP. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in ERP have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the ERP server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in ERP. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the ERP web
client.
...
user to modify it can be useful, but most fields in ERP also allow editing.
