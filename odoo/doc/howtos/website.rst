...
.. warning::
    ...
    * This guide assumes :ref:`an installed ERP <setup/install>`

Creating a basic module
=======================

In ERP, tasks are performed by creating modules.

Modules customize the behavior of an ERP installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`ERP's scaffolding <reference/cmdline/scaffold>` can setup a basic
...
display logic. ERP allows any Python templating system, but provides its
...
Finally restart ERP and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, ERP can be restarted :option:`and update modules at
...
:ref:`ERP models <reference/orm/model>` map to database tables.
...
ERP bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but ERP 8 added deeper
...
You're now in ERP "proper": the administrative interface. For now click on
...
restart ERP, access http://localhost:8069/academy/Alice/ and
...
Restart ERP, access http://localhost:8069/academy/2, note how the old value
...
ERP provides an additional converter called ``model`` which provides records
...
Restart ERP and upgrade the module, then you can visit each teacher's page.
...
Restart ERP and update the views, reload the teacher's page and… the field
...
Restart ERP and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the ERP administration
--------------------------------------------------------------

The ERP administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the ERP backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which ERP should
   execute or data display. Data display actions are called *window actions*,
   and tell ERP to display a given *model* according to a set of views…
...
Editing in the ERP administration
----------------------------------

By default, an ERP model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) ERP will automatically create a basic one
...
ERP provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of ERP's email and messaging
...
ERP also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the ERP system. We can easily make
...
restart ERP, update your module, there is now a :guilabel:`Shop` section in
