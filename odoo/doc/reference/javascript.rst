...
* Global selectors should be avoided. Because a component may be used several
  times in a single page (an example in ERP is dashboards), queries should be
  ...
  lead to unintended or incorrect behavior.  ERP Web's
...
To display and interact with data, calls to the ERP server are necessary.
This is performed using :abbr:`RPC <Remote Procedure Call>`.

ERP Web provides two primary APIs to handle this: a low-level
JSON-RPC based API communicating with the Python section of ERP
Web (and of your module, if you have a Python part) and a high-level
API above that allowing your code to talk directly to high-level ERP models.
...
High-level API: calling into ERP models
-------------------------------------------

Access to ERP object methods (made available through XML-RPC from the server)
is done via :class:`Model`. It maps onto the ERP server objects via two primary
...
the ERP server object. Its usage is similar to that of the ERP Model API,
...
interface to searches (``search`` + ``read`` in ERP RPC terms). It
...
ERP has powerful grouping capacities, but they are kind-of strange
...
ERP Web eschews direct calls to :py:meth:`~openerp.models.Model.read_group`
...
It has many advantages over the ERP version 8 system.
...
Testing in ERP Web Client
==========================

Javascript Unit Testing
-----------------------

ERP Web includes means to unit-test both the core code of
ERP Web and your own javascript modules. On the javascript side,
unit-testing is based on QUnit_ with a number of helpers and
extensions for better integration with ERP.

To see what the runner looks like, find (or start) an ERP server
...
``test`` key of the ERP manifest, by adding javascript files to it:
...
module. ERP tests live in a :func:`~openerp.testing.section`,
...
actually does its job. ERP Web test case use standard `QUnit
...
As noted above, ERP Web's tests use `qunit assertions`_. They are
...
Getting an ERP instance
------------------------

The ERP instance is the base through which most ERP Web
...
.. function:: mock(rpc_spec, handler)
    ...
    * If it matches the pattern ``model:method`` (if it contains a
      ...
      call straight to the ERP server (through XMLRPC) as
...
#. Install a new database with all relevant modules (all modules with
   ...
   .. note::
       ...
       being duplicated, but ERP doesn't currently break
...
.. note::
    ...
    but all the web tests for all of ERP. This can be surprising.
