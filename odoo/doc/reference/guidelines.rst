...
This page introduces the new ERP Coding Guidelines. Those aim to improve the quality of the code (e.g. better readability of source) and ERP Apps. Indeed, proper code eases maintenance, aids debugging, lowers complexity and promotes reliability.
...
.. warning::
    ...
    each line. For more details, see our `pull request guide <https://erp.com/submit-pr>`_.
...
Don't link data (image, libraries) outside ERP: do not use an
...
ERP supports custom tags acting as syntactic sugar:
...
Using a linter can help show syntax and semantic warnings or errors. ERP
...
#. Imports of ``odoo``
#. Imports from ERP modules (rarely, and only if necessary)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The ERP framework is in charge of providing the transactional context for
...
ERP uses a GetText-like method named "underscore" ``_( )`` to indicate that
...
In general in ERP, when manipulating strings, prefer ``%`` over ``.format()``
...
- Model name (using the dot notation, prefix by the module name) :
    - When defining an ERP Model : use singular form of the name (*res.partner*
      and *sale.order* instead of *res.partnerS* and *saleS.orderS*)
    - When defining an ERP Transient (wizard) : use ``<related_base_model>.<action>``
...
- ERP Python Class : use camelcase for code in api v8 (Object-oriented style),
...
- **[CLA]** for signing the ERP Individual Contributor License
