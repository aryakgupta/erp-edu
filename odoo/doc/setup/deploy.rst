...
This document describes basic steps to set up ERP in production. It follows
...
ERP is a multi-tenant system: a single ERP system may run and serve a number
...
bound to a database: ERP need to know which database should be used for the
...
accessible ERP needs a rule to know which one it should use.

That is one of the purposes of :option:`--db-filter <odoo.py --db-filter>`:
it specifies the default database for the ERP system. The value is a
`regular expression`_, possibly including the dynamically injected hostname
or subdomain through which the ERP system is accessed.

If an ERP hosts multiple databases in production, especially if ``website``
...
UNIX socket is fine if you want ERP and PostgreSQL to execute on the same
machine, and is the default when no host is provided, but if you want ERP and
...
* only accept loopback connections and `use an SSH tunnel`_ between the
  machine on which ERP runs and the one on which PostgreSQL runs, then
  configure ERP to connect to its end of the tunnel
* accept connections to the machine on which ERP is installed, possibly
  over ssl (see `PostgreSQL connection settings`_ for details), then configure
  ERP to connect over the network
...
Out of the box, ERP connects to a local postgres over UNIX socket via port
...
ERP includes built-in HTTP servers, using either multithreading or
multiprocessing.
...
* RAM = 9 * ((0.8*150) + (0.2*1024)) ~= 3Go RAM for ERP
...
Whether it's accessed via website/web client or the webservice, ERP transmits
authentication information in cleatext. This means a secure deployment of
ERP must use HTTPS\ [#switching]_. SSL termination can be implemented via
just about any SSL termination proxy, but requires the following setup:

* enable ERP's :option:`proxy mode <odoo.py --proxy-mode>`. This should only be enabled when ERP is behind a reverse proxy
...
ERP as a WSGI Application
==========================

It is also possible to mount ERP as a standard WSGI_ application. ERP
...
client, website and webservice API. Because ERP does not control the creation
of workers anymore it can not setup cron or livechat workers

Cron Workers
------------

To run cron jobs for an ERP deployment as a WSGI application requires

* a classical ERP (run via ``odoo.py``)
...
* deploy a threaded version of ERP (instread of a process-based preforking
  one) and redirect only requests to URLs starting with ``/longpolling/`` to
  that ERP, this is the simplest and the longpolling URL can double up as
  the cron instance.
* deploy an evented ERP via ``openerp-gevent`` and proxy requests starting
  with ``/longpolling/`` to
...
For development convenience, ERP directly serves all static files in its
...
ERP static files live in each module's ``static/`` folder, so static files
...
.. [#different-machines]
    to have multiple ERP installations use the same PostgreSQL database,
